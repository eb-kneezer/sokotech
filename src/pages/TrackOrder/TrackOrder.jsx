...
export default function TrackOrder() {
  ...
  const orderCost = pageOrder.items
    .reduce((init, item) => init + item.count * item.price, 0)
    .toFixed(2);
  const orderCount = pageOrder.items.reduce(
    (init, item) => init + item.count,
    0
  );
        ...
        <div className={styles.delivery}>
          <div>
            <h3 className={styles.deliveryName}>
              {pageOrder.deliveryAddress.name}
            </h3>
            <p>Cash on delivery</p>
          </div>
          <div>
            <p className={styles.deliveryPhone}>
              {pageOrder.deliveryAddress.mobile}
            </p>
            <p className={styles.deliveryAddress}>
              {pageOrder.deliveryAddress.address}
            </p>
          </div>
        </div>
      </div>
